...
// ########## METHOD PUBLIC
    ...
    ** <ul>
    ** <li>PGSQL_ASSOC: associative indexed array (fieldnames as indexes)</li>
    ...
    **                          - PGSQL_ASSOC associative indexed array (fieldnames as indexes)
    ...
    ** @returns !=0 on error
    **/
    public function fetchArray(&$aArray, &$rRslt, $iType=PGSQL_ASSOC)
    {
        $aArray = pg_fetch_array($rRslt, null, $iType);
        if ($aArray === false) $aArray = null;
    ...
    **/
    ...
    {
        $iErr = 0;
        if(false === ($rRslt = pg_query($this->_rLink, $sQuery))){
            $iErr = 1; // query failed
            lib\ErrLog::getInstance()->push("{".get_class($this)."} _query[".pg_last_error($this->_rLink)."] Query[".$sQuery."]");
        }
        return($iErr);
    }
    /**
    ...
    ** @return 0 on success !0 on error
    **/
    public function queryOneRow(&$aRow, $sQuery, $iType=PGSQL_ASSOC)
    {
        $iErr  = 0;
        $rRslt = false;
        $aRow  = NULL;
        ...
        else if(0!=($iErr = $this->fetchArray($aRow, $rRslt, PGSQL_ASSOC)) || NULL===$aRow);
    ...
    **/
    ...
    {
        return(pg_escape_string($this->_rLink, $s));
    }
    /**
...
// MISC
    ...
    **/
    ...
    {
        $result = pg_query($this->_rLink, "SELECT version()");
...
// ########## METHOD PROTECTED
    ...
    **/
    ...
    {
        $iErr = 0;
        $conn_string = "host=".$this->_sHost." port=".$this->_sPort." dbname=".$this->_sDataBase." user=".$this->_sUser." password=".$this->_sPass;
        if(false===($this->_rLink = pg_connect($conn_string))){
            $iErr = 1; // failed to connect to host
            lib\ErrLog::getInstance()->push("{".get_class($this)."} _connect: connect to ".$this->_sHost."[".pg_last_error()."]");
        }
        return($iErr);
    }
// ########## METHOD PRIVATE
